...
- 附件区支持带放置位置提示的拖动排序；编辑者逐件决定是否插入正文，紧凑卡片会随桌面窄栏和手机宽度重排。
- 图片、PDF、文本及 DOCX、XLSX、PPTX 等支持格式可在正文原位展开，多份材料可同时阅读；PDF 与 Office 文件另有沉浸式独立预览。
- Office 文件只在当前浏览器解析，不发送至 Microsoft 或其他第三方；加载过程提供阶段、进度、超时重试与下载兜底。
- 有编辑权限的用户可从正式内容页点击“点击编辑”直达已获取最新数据的编辑器；修改记录同步覆盖附件变更。
- 系统管理员可调整公开资料顺序和单文件上传上限；公开资料 Office 文件同步支持预览，关键管理页面补齐刷新操作。
- 新提醒弹条可直接打开通知中心对应详情；进入主要子页时静默获取最新数据。
